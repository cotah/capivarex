...
FOLLOWUP_PATTERNS = [
    # Health
    (
        r"(?:vou|tenho|marquei|agendei)\s+(?:ao|no|uma?\s+consulta|médico|dentista|exame|cirurgia)",
        "health",
        1,
    ),
    (
        r"(?:doctor|dentist|appointment|surgery|exam|checkup|hospital)\s*(?:tomorrow|next|on)",
        "health",
        1,
    ),
    # Job/Career
    (
        r"(?:entrevista|interview)\s+(?:de|d[eo]|for)\s+(?:emprego|trabalho|job|work)",
        "career",
        1,
    ),
    (r"(?:entrevista|interview)\s+(?:amanhã|tomorrow|next)", "career", 1),
    (
        r"(?:prova|exame|test|exam)\s+(?:é\s+)?(?:amanhã|tomorrow|next|na|no)",
        "education",
        1,
    ),
    # Travel
    (
        r"(?:viajo|vou viajar|trip|flying|voo|flight)\s+(?:amanhã|tomorrow|next|para|to|on)",
        "travel",
        1,
    ),
    (
        r"(?:volto|voltando|returning|coming back)\s+(?:amanhã|tomorrow|next)",
        "travel",
        1,
    ),
    # Social
    (
        r"(?:casamento|wedding|aniversário|birthday party)\s+(?:amanhã|tomorrow|next|do|da|no|na)",
        "social",
        1,
    ),
    ...
    # Work
    (
        r"(?:apresentação|presentation|pitch|demo)\s+(?:amanhã|tomorrow|next|important)",
        "work",
        1,
    ),
...
async def store_followup(
    ...
    try:
        ...
        existing = [
            f
            for f in existing
            if not f.get("done") or time.time() - f.get("created_at", 0) < 604800
        ]
        ...
        logger.info(
            "Smart follow-up stored: user=%s cat=%s in %d days",
            user_id[:8],
            event.get("category"),
            days,
        )
...
async def _generate_ai_followup(
    name: str, category: str, original: str
) -> Optional[str]:
...
def _generate_fallback_followup(name: str, category: str, original: str) -> str:
    ...
    return templates.get(
        category,
        f"{greeting} 💭 Lembrei que você mencionou algo. Como está? Tudo certo? 😊",
    )
