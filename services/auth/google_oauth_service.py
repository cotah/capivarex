...
class GoogleOAuthService:
    ...
    async def handle_callback(
        ...
        """
        # 1. Decodificar state (undo possible double URL-encoding)
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 200:
                # Handle invalid_grant gracefully (expired/reused code)
                if resp.status_code == 400:
                    ...
                    if error_data.get("error") == "invalid_grant":
                        logger.warning(
                            "Google OAuth: invalid_grant — code expired or already used"
                        )
                        return {
                            "error": "authorization_expired",
                            "message": "Authorization expired. Please try connecting Google again.",
                        }
