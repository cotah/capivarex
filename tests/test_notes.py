...
class TestNotesServiceHelpers:
    ...
    def test_short_id(self):
        ...
        assert (
            NotesService.short_id("abc12345-0000-0000-0000-000000000000") == "abc12345"
        )
...
class TestNotesServiceValidation:
    ...
    async def test_create_limit_reached_raises(self):
        ...
        svc = self._svc()
        with patch.object(
            svc, "_count_notes", new=AsyncMock(return_value=MAX_NOTES_PER_USER)
        ):
...
class TestNotesAgentExecute:
    ...
    async def test_list_with_notes(self):
        ...
        svc.list_notes = AsyncMock(
            return_value=[
                _make_note(),
                _make_note("def67890-0000-0000-0000-000000000000"),
            ]
        )
        with patch("agents.specialized.notes_agent.get_service", return_value=svc):
    ...
    async def test_list_empty(self):
        ...
        assert r.status.value == "success"
        assert (
            "não tens" in r.response.lower()
            or "sem notas" in r.response.lower()
            or "não" in r.response.lower()
        )
