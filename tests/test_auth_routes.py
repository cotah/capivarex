...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_resolve_user_uuid_non_numeric():
    """resolve_user_uuid returns non-numeric IDs unchanged."""
    from utils.identity import resolve_user_uuid

    result = await resolve_user_uuid("abc-uuid-123")
    assert result == "abc-uuid-123"


@pytest.mark.unit
@pytest.mark.asyncio
async def test_resolve_user_uuid_numeric_no_db():
    """resolve_user_uuid returns numeric ID when DB service unavailable."""
    from utils.identity import resolve_user_uuid

    with patch("services.core.get_service", return_value=None):
        result = await resolve_user_uuid("6316076982")
    assert result == "6316076982"


@pytest.mark.unit
@pytest.mark.asyncio
async def test_resolve_user_uuid_numeric_with_db():
    """resolve_user_uuid resolves numeric telegram_id to UUID."""
    from utils.identity import resolve_user_uuid
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_resolve_user_uuid_numeric_not_found():
    """resolve_user_uuid returns original ID when telegram_id not in DB."""
    from utils.identity import resolve_user_uuid
    ...
    with patch("services.core.get_service", return_value=mock_db):
        result = await resolve_user_uuid("9999999")
    assert result == "9999999"


@pytest.mark.unit
@pytest.mark.asyncio
async def test_resolve_user_uuid_db_exception():
    """resolve_user_uuid handles DB exceptions gracefully."""
    from utils.identity import resolve_user_uuid
