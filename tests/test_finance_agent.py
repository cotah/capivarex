...
def mock_finance_svc():
    ...
    svc.get_quote = AsyncMock(return_value={
        "symbol": "AAPL",
        "name": "Apple Inc.",
        "price": 195.50,
        "currency": "USD",
...
async def test_finance_quote_response(finance_agent, mock_finance_svc):
    ...
    with patch("agents.specialized.finance_agent.get_service", return_value=mock_finance_svc):
...
async def test_finance_service_unavailable(finance_agent):
    ...
    assert result.status == AgentStatus.ERROR
    assert "disponivel" in result.response.lower()
