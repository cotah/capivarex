...
class TestCounters:
    ...
    async def test_events_no_db(self):
        with patch(
            "services.business.weekly_wins_service.get_service", return_value=None
        ):
    ...
    async def test_events_with_data(self):
        ...
        mock_db.get_client.return_value.table.return_value.select.return_value.eq.return_value.gte.return_value.lte.return_value.execute.return_value = MagicMock(
            count=5
        )
        with patch(
            "services.business.weekly_wins_service.get_service", return_value=mock_db
        ):
    ...
    async def test_conversations_with_data(self):
        mock_db = MagicMock()
        mock_db.is_initialized.return_value = True
        mock_db.get_client.return_value.table.return_value.select.return_value.eq.return_value.gte.return_value.execute.return_value = MagicMock(
            count=15
        )
        with patch(
            "services.business.weekly_wins_service.get_service", return_value=mock_db
...
class TestGenerateWeeklyWins:
    ...
    async def test_full_flow(self):
        with (
            patch(
                "services.business.weekly_wins_service._count_week_events",
                new_callable=AsyncMock,
                return_value=5,
            ),
            patch(
                "services.business.weekly_wins_service._count_completed_tasks",
                new_callable=AsyncMock,
                return_value=3,
            ),
            patch(
                "services.business.weekly_wins_service._count_conversations",
                new_callable=AsyncMock,
                return_value=10,
            ),
            patch(
                "services.business.weekly_wins_service._get_week_focus_time",
                new_callable=AsyncMock,
                return_value=120,
            ),
            patch(
                "services.business.weekly_wins_service._generate_ai_wins",
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch(
                "services.business.weekly_wins_service._store_wins",
                new_callable=AsyncMock,
            ),
        ):
