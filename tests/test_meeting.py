...
def mock_cal_svc():
    ...
    svc.async_create_meeting = AsyncMock(
        return_value={
            "id": "event_abc123",
...
class TestMeetingAgentCreate:
    ...
    async def test_create_meeting_basic(self, meeting_agent, mock_cal_svc):
        with patch(
            "agents.specialized.meeting_agent.get_service", return_value=mock_cal_svc
        ):
            r = await meeting_agent.execute(
                "Cria uma reunião com joao@email.com amanhã às 10h", {}
            )
        assert r.is_success()
        mock_cal_svc.async_create_meeting.assert_called_once()
        # Verifica que o email foi detectado
        call_kwargs = mock_cal_svc.async_create_meeting.call_args.kwargs
        assert "joao@email.com" in call_kwargs.get("attendees", [])

    @pytest.mark.asyncio
    async def test_create_meeting_meet_link_in_response(
        self, meeting_agent, mock_cal_svc
    ):
        with patch(
            "agents.specialized.meeting_agent.get_service", return_value=mock_cal_svc
        ):
    ...
    @pytest.mark.asyncio
    async def test_create_meeting_default_duration_60min(
        self, meeting_agent, mock_cal_svc
    ):
    ...
    async def test_create_meeting_context_override(self, meeting_agent, mock_cal_svc):
        ...
        # 90 minutos
        delta = (
            call_kwargs["end_time"] - call_kwargs["start_time"]
        ).total_seconds() / 60
        assert delta == 90
...
class TestMeetingAgentErrors:
    ...
    async def test_service_unavailable(self, meeting_agent):
        with patch("agents.specialized.meeting_agent.get_service", return_value=None):
            r = await meeting_agent.execute("Cria reunião amanhã às 10h", {})
    ...
    async def test_create_meeting_returns_none(self, meeting_agent, mock_cal_svc):
        ...
        mock_cal_svc.async_create_meeting.return_value = None
        with patch(
            "agents.specialized.meeting_agent.get_service", return_value=mock_cal_svc
        ):
            r = await meeting_agent.execute("Cria reunião amanhã às 10h", {})
    ...
    async def test_calendar_service_exception(self, meeting_agent, mock_cal_svc):
        ...
        mock_cal_svc.async_create_meeting.side_effect = RuntimeError("Google API error")
        with patch(
            "agents.specialized.meeting_agent.get_service", return_value=mock_cal_svc
        ):
            r = await meeting_agent.execute("Cria reunião amanhã às 10h", {})
...
class TestMeetingAgentResponse:
    ...
    @pytest.mark.asyncio
    async def test_response_no_meet_link_still_succeeds(
        self, meeting_agent, mock_cal_svc
    ):
        """Se por algum motivo o Meet link não vier, ainda retorna sucesso."""
        mock_cal_svc.async_create_meeting.return_value = {
            "id": "event_xyz",
            "summary": "Reunião",
