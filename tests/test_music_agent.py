...
class TestFormatTracksResponse:
    ...
    def test_single_track_pt(self, sample_track):
        result = _format_tracks_response([sample_track], lang="pt")
...
class TestFormatArtistResponse:
    def test_format_pt(self, sample_artist):
        result = _format_artist_response(sample_artist, lang="pt")
        assert "Queen" in result
    ...
    def test_format_en(self, sample_artist):
        result = _format_artist_response(sample_artist, lang="en")
        assert "Queen" in result
    ...
    def test_format_es(self, sample_artist):
        result = _format_artist_response(sample_artist, lang="es")
        assert "Queen" in result
        assert "seguidores" in result
...
class TestFormatAlbumResponse:
    def test_format_pt(self):
        ...
        result = _format_album_response(album, lang="pt")
    ...
    def test_format_en(self):
        ...
        result = _format_album_response(album, lang="en")
    ...
    def test_format_es(self):
        ...
        result = _format_album_response(album, lang="es")
...
def _make_openai_mock(intent_json: str):
    ...
    mock_completion.choices = [MagicMock(message=MagicMock(content=intent_json))]
    mock.client = MagicMock()
    mock.client.chat.completions.create = AsyncMock(return_value=mock_completion)
    return mock


class TestMusicAgentExecution:
    @pytest.mark.asyncio
    async def test_search_track_pt(self, music_agent, mock_spotify, sample_track):
        """Search track returns PT response."""
        mock_spotify.search_tracks = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
            '{"action": "search_track", "query": "Bohemian Rhapsody", "language": "pt"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("busca Bohemian Rhapsody", {})
    ...
    @pytest.mark.asyncio
    async def test_search_track_en(self, music_agent, mock_spotify, sample_track):
        """Search track returns EN response."""
        mock_spotify.search_tracks = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
            '{"action": "search_track", "query": "Bohemian Rhapsody", "language": "en"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("search Bohemian Rhapsody", {})

        assert result.status == AgentStatus.SUCCESS
        assert "Listen on Spotify" in result.response

    @pytest.mark.asyncio
    async def test_search_track_es(self, music_agent, mock_spotify, sample_track):
        """Search track returns ES response."""
        mock_spotify.search_tracks = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
            '{"action": "search_track", "query": "Bohemian Rhapsody", "language": "es"}'
        )
    ...
    async def test_search_artist_success(
        ...
        mock_spotify.search_artists = AsyncMock(return_value=[sample_artist])
        # Always fetches full profile after search
        mock_spotify.get_artist = AsyncMock(return_value=sample_artist)
        mock_openai = _make_openai_mock(
            '{"action": "search_artist", "query": "Queen", "language": "pt"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("quem e Queen", {})
    ...
    @pytest.mark.asyncio
    async def test_search_artist_always_fetches_full(self, music_agent, mock_spotify):
        ...
        mock_spotify.search_artists = AsyncMock(return_value=[sparse_artist])
        mock_spotify.get_artist = AsyncMock(return_value=full_artist)
        mock_openai = _make_openai_mock(
            '{"action": "search_artist", "query": "Eminem", "language": "en"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("who is Eminem", {})
        ...
        mock_spotify.get_artist.assert_called_once_with("a1")

    @pytest.mark.asyncio
    async def test_spotify_unavailable(self, music_agent):
    ...
    @pytest.mark.asyncio
    async def test_no_results_pt(self, music_agent, mock_spotify):
        """Returns PT message when no results found."""
        mock_spotify.search_tracks = AsyncMock(return_value=[])
        mock_openai = _make_openai_mock(
            '{"action": "search_track", "query": "xyz", "language": "pt"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("busca xyz", {})

        assert result.status == AgentStatus.SUCCESS
        assert "Nenhuma" in result.response

    @pytest.mark.asyncio
    async def test_no_results_en(self, music_agent, mock_spotify):
        """Returns EN message when no results found."""
        mock_spotify.search_tracks = AsyncMock(return_value=[])
        mock_openai = _make_openai_mock(
            '{"action": "search_track", "query": "xyz", "language": "en"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("search xyz", {})

        assert result.status == AgentStatus.SUCCESS
        assert "No music found" in result.response

    @pytest.mark.asyncio
    async def test_exception_handling(self, music_agent, mock_spotify):
        """Agent handles exceptions gracefully."""
        mock_spotify.search_tracks = AsyncMock(side_effect=RuntimeError("API down"))
    ...
    @pytest.mark.asyncio
    async def test_recommendations_pt(self, music_agent, mock_spotify, sample_track):
        """Recommendations in PT."""
        mock_spotify.get_recommendations = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
            '{"action": "recommendations", "genre": "rock", "language": "pt"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("recomenda rock", {})

        assert result.status == AgentStatus.SUCCESS
        assert "Recomendações" in result.response

    @pytest.mark.asyncio
    async def test_recommendations_en(self, music_agent, mock_spotify, sample_track):
        """Recommendations in EN."""
        mock_spotify.get_recommendations = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
            '{"action": "recommendations", "genre": "rock", "language": "en"}'
        )
        ...
        with patch(
            ...
            result = await music_agent.execute("recommend rock", {})

        assert result.status == AgentStatus.SUCCESS
        assert "Recommendations" in result.response

    @pytest.mark.asyncio
    async def test_artist_top_tracks(self, music_agent, mock_spotify, sample_track):
        """Top tracks uses search-based method."""
        mock_spotify.get_artist_top_tracks = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock(
        ...
        with patch(
            ...
            result = await music_agent.execute("top musicas do Eminem", {})
    ...
    async def test_lang_fallback_to_context(
        ...
        mock_spotify.search_tracks = AsyncMock(return_value=[sample_track])
        mock_openai = _make_openai_mock('{"action": "search_track", "query": "test"}')
