...
class TestNotificationFlags:
    ...
    def test_should_notify_after_cooldown(self, sample_ticket):
        old_ts = (datetime.now() - timedelta(minutes=COOLDOWN_MINUTES + 5)).isoformat(
            timespec="seconds"
        )
    ...
    def test_should_notify_after_status_change(self, sample_ticket):
        ...
        recent_notif = (datetime.now() - timedelta(minutes=10)).isoformat(timespec="seconds")
        recent_change = (datetime.now() - timedelta(minutes=2)).isoformat(timespec="seconds")
