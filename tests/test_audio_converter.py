...
class TestMp3ToMulaw:
    ...
    def test_mp3_to_mulaw_chunks_returns_base64_list(self):
        """mp3_to_mulaw_chunks returns list of base64 strings."""
        # Skip if pydub/ffmpeg not available
        pytest.importorskip("pydub")

        from utils.audio_converter import mp3_to_mulaw_chunks

        # Use WAV as input (pydub can handle it even though function says mp3)
        # For real test, would need actual MP3 — but WAV tests the pipeline
        try:
            from pydub import AudioSegment

            # Create a tiny WAV and convert to MP3
            wav_bytes = self._make_wav_bytes(0.1)
            audio = AudioSegment.from_wav(io.BytesIO(wav_bytes))
            mp3_buf = io.BytesIO()
            audio.export(mp3_buf, format="mp3")
            mp3_bytes = mp3_buf.getvalue()

            chunks = mp3_to_mulaw_chunks(mp3_bytes)
            ...
            pytest.skip("ffmpeg not available for MP3 conversion")
    ...
    def test_mp3_to_mulaw_raw_returns_bytes(self):
        """mp3_to_mulaw_raw returns raw mulaw bytes."""
        pytest.importorskip("pydub")
        ...
        try:
            ...
            mulaw = mp3_to_mulaw_raw(mp3_bytes)
