...
def mock_weather_svc():
    ...
    svc.get_week_summary = AsyncMock(
        return_value={
...
async def test_weather_current_success(weather_agent, mock_weather_svc):
    ...
    with patch(
        "agents.specialized.weather_agent.get_service", return_value=mock_weather_svc
    ):
        result = await weather_agent.execute(
            "como está o tempo em Dublin?", {"location": "Dublin"}
        )
...
async def test_weather_service_exception(weather_agent, mock_weather_svc):
    ...
    mock_weather_svc.get_detailed_forecast = AsyncMock(
...
@pytest.mark.asyncio
async def test_weather_empty_prompt_uses_default_location(
    weather_agent, mock_weather_svc
):
