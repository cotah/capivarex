...
def mock_elevenlabs_service():
    ...
    svc = AsyncMock()
    svc.is_initialized.return_value = True
    svc.text_to_speech = AsyncMock(return_value=b"\xFF\xFB\x90" * 1000)  # fake MP3 bytes
    return svc
...
class TestVoicePipelineServiceHappyPath:
    ...
    async def test_process_audio_full_pipeline(
        ...
        assert result["response_text"] == "Hoje o tempo estará ensolarado com temperatura de 28°C."
...
class TestVoicePipelineFailSafe:
    ...
    @pytest.mark.asyncio
    async def test_llm_failure_stops_pipeline(
        self, pipeline_service, fake_audio_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_whisper_service_unavailable(self, mock_elevenlabs_service, fake_audio_file):
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_unavailable(
        self, pipeline_service, fake_audio_file
    ):
...
class TestVoicePipelineEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_stt_only_without_elevenlabs(self, mock_whisper_service, fake_audio_file):
    ...
    async def test_english_language_uses_english_voice(
        ...
        call_args = pipeline_service._elevenlabs.text_to_speech.call_args
        voice_used = call_args.kwargs.get("voice_id") or (call_args.args[1] if len(call_args.args) > 1 else None)
...
class TestVoicePipelineRoutes:
    ...
    def client(self, pipeline_service, mock_orchestrator):
        ...
        with patch("api.routes.voice_pipeline_routes.get_service", return_value=pipeline_service), \
...
class TestElevenLabsService:
    ...
    async def test_text_to_speech_success(self):
        ...
        with patch("services.ai.elevenlabs_service.httpx.AsyncClient", return_value=mock_client):
