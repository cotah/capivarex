...
def mock_svc():
    ...
    svc.search_places = AsyncMock(
        return_value={
            "places": [
                {
                    "title": "Cantina Italiana",
                    "address": "1 Grafton St, Dublin",
                    "phone": "+353 1 000 0001",
                    "rating": 4.8,
                    "reviews": 320,
                    "type": "Restaurant",
                    "website": "https://cantina.ie",
                    "open_now": "Open now",
                    "coordinates": {"lat": 53.33, "lng": -6.26},
...
class TestSearchAgentIntents:
    ...
    async def test_places_intent(self, search_agent, mock_svc):
        with patch(
            ...
            r = await search_agent.execute("restaurante italiano perto de mim", {})
        assert r.is_success()
    ...
    async def test_general_shows_answer_box(self, search_agent, mock_svc):
        with patch(
            ...
            r = await search_agent.execute("Bitcoin price", {"search_type": "general"})
    ...
    async def test_context_search_type_override(self, search_agent, mock_svc):
        ...
        with patch(
            ...
            await search_agent.execute("Python", {"search_type": "shopping"})
        mock_svc.search_shopping.assert_called_once()
    ...
    async def test_places_empty_returns_error(self, search_agent, mock_svc):
        ...
        assert (
            "no places found" in r.response.lower()
            or "não encontrei" in r.response.lower()
        )
    ...
    async def test_empty_query_returns_help(self, search_agent, mock_svc):
        ...
        assert (
            ...
            or "buscar" in r.response.lower()
            or "busca" in r.response.lower()
        )
...
class TestSearchAgentErrors:
    ...
    async def test_api_key_error_friendly(self, search_agent, mock_svc):
        mock_svc.search_places.side_effect = RuntimeError("SERPER_API_KEY inválida.")
        with patch(
            "agents.specialized.search_agent.get_service", return_value=mock_svc
        ):
            r = await search_agent.execute("restaurante perto", {})
    ...
    async def test_quota_error_friendly(self, search_agent, mock_svc):
        mock_svc.search_places.side_effect = RuntimeError("Quota Serper esgotada.")
        with patch(
            "agents.specialized.search_agent.get_service", return_value=mock_svc
        ):
            r = await search_agent.execute("restaurante perto", {})
...
class TestSearchAgentImages:
    ...
    async def test_images_empty_returns_error(self, search_agent, mock_svc):
        ...
        assert not r.is_success()
        assert (
            "no images found" in r.response.lower()
            or "não encontrei" in r.response.lower()
        )
