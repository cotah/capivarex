...
def _px(mock_client):
    """Shorthand to patch httpx.AsyncClient where RedisService uses it."""
    return patch(
...
class TestRedisInit:
    ...
    async def test_init_missing_token(self):
        ...
        with patch.dict(
            os.environ,
            {"UPSTASH_REDIS_REST_URL": "url", "UPSTASH_REDIS_REST_TOKEN": ""},
            clear=False,
        ):
            with pytest.raises(ServiceUnavailableError):
                await svc._initialize()
    ...
    async def test_health_check_success(self):
        ...
        mock_client.post = AsyncMock(return_value=_mock_response({"result": "PONG"}))
        with _px(mock_client):
    ...
    async def test_cleanup(self):
        """cleanup is now a no-op since clients are created per-request."""
        svc, _mock_client = _make_service()
        await svc._cleanup()
        # No client to close — just verify no exception is raised
...
class TestExecuteCommand:
    ...
    async def test_success(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": "OK"}))
        with _px(mock_client):
    ...
    async def test_http_error(self):
        ...
        mock_client.post = AsyncMock(
            return_value=_mock_response({"error": "bad"}, status_code=400)
        )
        with _px(mock_client):
...
class TestCoreCommands:
    ...
    async def test_get_string(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": "hello"}))
        with _px(mock_client):
    ...
    async def test_get_json(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(
            return_value=_mock_response({"result": '{"a": 1}'})
        )
        with _px(mock_client):
            result = await svc.get("key")
        assert result == {"a": 1}
    ...
    async def test_get_none(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": None}))
        with _px(mock_client):
            result = await svc.get("key")
    ...
    async def test_delete(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": 1}))
        with _px(mock_client):
    ...
    async def test_delete_nonexistent(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": 0}))
        with _px(mock_client):
            result = await svc.delete("key")
        assert result is False
    ...
    async def test_get_ttl(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": 300}))
        with _px(mock_client):
...
class TestListCommands:
    ...
    async def test_lpush(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": 3}))
        with _px(mock_client):
    ...
    async def test_lrange(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(
            return_value=_mock_response({"result": ["a", "b", "c"]})
        )
        with _px(mock_client):
...
class TestConversationCache:
    ...
    async def test_get_conversation_context(self):
        ...
        mock_client.post = AsyncMock(
            return_value=_mock_response({"result": json_dumps(msgs)})
        )
        with _px(mock_client):
...
class TestConversationHistory:
    ...
    async def test_set_history(self):
        ...
        mock_client.post = AsyncMock(return_value=_mock_response({"result": "OK"}))
        with _px(mock_client):
            result = await svc.set_conversation_history(
...
class TestSessionManagement:
    ...
    async def test_get_session(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(
            return_value=_mock_response({"result": '{"user_id": "u1"}'})
        )
        with _px(mock_client):
...
class TestPipeline:
    ...
    async def test_pipeline_execution(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(
            return_value=_mock_response([{"result": "OK"}, {"result": "value1"}])
        )
        with _px(mock_client):
...
class TestRedisInitAdvanced:
    ...
    async def test_health_check_non_pong(self):
        ...
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(return_value=_mock_response({"result": "WRONG"}))
        with _px(mock_client):
...
class TestCoreCommandErrors:
    ...
    async def test_set_error(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(side_effect=RuntimeError("connection lost"))
        with _px(mock_client):
...
class TestConversationCacheErrors:
    ...
    async def test_save_message_error(self):
        ...
        with _px(mock_client):
            with pytest.raises(
                RuntimeError, match="Failed to save conversation message"
            ):
    ...
    async def test_get_context_error(self):
        ...
        with _px(mock_client):
            with pytest.raises(
                RuntimeError, match="Failed to get conversation context"
            ):
    ...
    async def test_refresh_ttl_error(self):
        ...
        with _px(mock_client):
            with pytest.raises(
                RuntimeError, match="Failed to refresh conversation TTL"
            ):
...
class TestPipelineErrors:
    ...
    async def test_pipeline_http_error(self):
        svc, mock_client = _make_service()
        mock_client.post = AsyncMock(
            return_value=_mock_response({"error": "bad"}, status_code=500)
        )
        with _px(mock_client):
