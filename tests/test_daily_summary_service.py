...
class TestGetTodayEvents:
    ...
    async def test_no_db(self):
        with patch("services.business.daily_summary_service.get_service", return_value=None):
    ...
    async def test_with_events(self):
        ...
        mock_db.get_client.return_value.table.return_value.select.return_value.eq.return_value.gte.return_value.lte.return_value.order.return_value.limit.return_value.execute.return_value = MagicMock(
            data=[{"title": "Standup", "start_time": "2026-03-17T09:00:00", "end_time": "2026-03-17T09:30:00"}]
        )
        with patch("services.business.daily_summary_service.get_service", return_value=mock_db):
...
class TestGetFocusTime:
    ...
    @pytest.mark.asyncio
    async def test_exception(self):
        with patch("services.business.focus_mode_service.get_focus_state", new_callable=AsyncMock, side_effect=Exception("err")):
            assert await _get_focus_time_today("u1") == 0
...
class TestFallbackSummary:
    def test_full_day(self):
        ...
        assert "2 eventos" in msg
        assert "1 tarefa pendente" in msg
        assert "1h30min" in msg
        assert "Dentist" in msg

    def test_empty_day(self):
        data = {"events_today": [], "pending_tasks": [], "events_tomorrow": [], "focus_minutes": 0}
        msg = _generate_fallback_summary("", data)
        assert "Nenhum evento" in msg
        assert "agenda livre" in msg

    def test_focus_minutes_only(self):
        data = {"events_today": [], "pending_tasks": [], "events_tomorrow": [], "focus_minutes": 25}
    ...
    def test_multiple_pending(self):
        ...
        msg = _generate_fallback_summary("", data)
        assert "3 tarefas pendentes" in msg
...
class TestGenerateDailySummary:
    ...
    async def test_full_flow(self):
        with (
            patch("services.business.daily_summary_service._get_today_events", new_callable=AsyncMock, return_value=[{"title": "Meeting"}]),
            patch("services.business.daily_summary_service._get_tomorrow_events", new_callable=AsyncMock, return_value=[]),
            patch("services.business.daily_summary_service._get_pending_tasks", new_callable=AsyncMock, return_value=[]),
            patch("services.business.daily_summary_service._get_focus_time_today", new_callable=AsyncMock, return_value=0),
            patch("services.business.daily_summary_service._generate_ai_summary", new_callable=AsyncMock, return_value=None),
            patch("services.business.daily_summary_service._store_summary", new_callable=AsyncMock),
        ):
            result = await generate_daily_summary("u1", "João")
        assert result is not None
        assert "Resumo do dia" in result["text"]
        assert result["data"]["events_today"] == [{"title": "Meeting"}]

    @pytest.mark.asyncio
    async def test_with_ai(self):
        with (
            patch("services.business.daily_summary_service._get_today_events", new_callable=AsyncMock, return_value=[]),
            patch("services.business.daily_summary_service._get_tomorrow_events", new_callable=AsyncMock, return_value=[]),
            patch("services.business.daily_summary_service._get_pending_tasks", new_callable=AsyncMock, return_value=[]),
            patch("services.business.daily_summary_service._get_focus_time_today", new_callable=AsyncMock, return_value=60),
            patch("services.business.daily_summary_service._generate_ai_summary", new_callable=AsyncMock, return_value="📊 AI generated summary here"),
            patch("services.business.daily_summary_service._store_summary", new_callable=AsyncMock),
        ):
...
class TestAISummary:
    ...
    async def test_ai_success(self):
        ...
        mock_openai.chat_completion.return_value = "📊 Resumo: dia produtivo com 3 reuniões!"
        with patch("services.business.daily_summary_service.get_service", return_value=mock_openai):
            result = await _generate_ai_summary("João", {
                "events_today": [{"title": "Meeting"}],
                "pending_tasks": [],
                "events_tomorrow": [],
                "focus_minutes": 60,
