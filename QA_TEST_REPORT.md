...
| ANTHROPIC_API_KEY          | SET     | Dev Agent (Claude)                   |
| SONAR_API_KEY              | SET     | Research Agent (Perplexity)           |
| TWELVE_DATA_API_KEY        | SET     | Finance Agent (Twelve Data)           |

**Note:** All API keys are configured in Railway. Previous report incorrectly listed PERPLEXITY_API_KEY and ALPHA_VANTAGE_API_KEY which do not exist in the codebase. The actual env vars are SONAR_API_KEY and TWELVE_DATA_API_KEY.
...
| research      | LOADED  | Web search (SONAR_API_KEY)           |
...
| finance       | LOADED  | Stock quotes (TWELVE_DATA_API_KEY)   |
...
|--------------|----------|----------------|------------------------------------------------|--------------|
| BUG-API-001  | CRITICAL | API REST       | `/chat/stream` 500 error - slowapi `request` parameter conflict | FIXED (params renamed: request=Request, body=ChatStreamRequest) |
| BUG-API-002  | MEDIUM   | API REST       | Duplicated route path prefixes (weather/weather, finance/finance, car/car) | FIXED (not present in current code) |
| BUG-ENV-001  | MEDIUM   | Environment    | 3 API keys now configured (ANTHROPIC, SONAR, TWELVE_DATA) | FIXED |
| BUG-TBOT-001 | LOW      | Telegram Bot   | UnicodeEncodeError on Windows cp1252 console   | FIXED        |
| BUG-API-003  | LOW      | API REST       | Rate limiting untestable due to BUG-API-001    | UNBLOCKED (BUG-API-001 fixed) |

### 7.2 Critical Bug Detail

#### BUG-API-001: Chat Stream Endpoint Crash -- FIXED

- **Endpoint:** `POST /api/v1/chat/stream`
- **File:** `api/routes/chat.py:269-275`
- **Original Issue:** slowapi required `request` parameter to be `starlette.requests.Request`, but it was a Pydantic model.
- **Fix Applied:** Parameters renamed correctly:
  ```python
  @router.post("/stream")
  @limiter.limit("10/minute")
  async def chat_stream(
      request: Request,               # starlette Request (required by slowapi) -- FIXED
      body: ChatStreamRequest,        # Pydantic model body -- FIXED
      processor: RequestProcessor = Depends(get_authenticated_processor),
  ) -> StreamingResponse:
  ```
  All internal references updated to use `body.message`, `body.history`, etc.
- **Status:** FIXED in current codebase. QA report was written against a previous version.
...
- ~~Fix BUG-API-001 (critical chat endpoint broken)~~ (FIXED)
- ~~Normalize route prefixes (BUG-API-002)~~ (FIXED - not present in current code)
- ~~Add missing API keys for full coverage~~ (FIXED)
...
1. ~~**Fix BUG-API-001**~~ - FIXED: Parameters already renamed correctly in current codebase (`request: Request`, `body: ChatStreamRequest`).

### Priority 2 (Short-term)
2. ~~**Fix BUG-API-002**~~ - FIXED: Route prefixes are correct in current codebase. No duplication found.
3. ~~**Add missing API keys**~~ - FIXED: All 3 keys configured in Railway (`ANTHROPIC_API_KEY`, `SONAR_API_KEY`, `TWELVE_DATA_API_KEY`).
