...
_RE_VER = re.compile(
    r"^(?:ver|ve|mostra[r]?|lista[r]?)\s+(?:a\s+)?(?:lista|compras)"
    r"|(?:o\s+que\s+tem|minha\s+lista|lista\s+de\s+compras?|quais\s+itens)"
    r"|(?:check\s+(?:my\s+)?list|checklist|show\s+my\s+list"
    r"|what(?:'|'|\u2019)?s\s+on\s+my\s+list|see\s+my\s+list|my\s+shopping\s+list)",
    re.IGNORECASE,
)
        ...
        try:
            ...
            if is_voice:
                ...
                if items and len(items) >= 1:
                    ...
                    result = await svc.adicionar(items_str, user_id=user_id, lang=lang)
            ...
            if texto and len(texto) < 150:
                # Guard: if ALL words are command-like (not products),
                # show the list instead of adding them as items.
                words = set(texto.lower().split())
                if words and words.issubset(_NON_PRODUCT_WORDS):
                    result = await svc.ver_lista(user_id=user_id, lang=lang)
                    return AgentResponse(
                        status=AgentStatus.SUCCESS,
                        response=result.get(
