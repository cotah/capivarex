...
"genres", "general_info", "play_track", "pause", "next", \
"previous", "volume", "now_playing", "add_to_playlist", \
"connect_spotify"
...
- "play_track": User wants to PLAY a specific track/album \
NOW on their device. Examples: "toca candy shop", \
"play bohemian rhapsody", "reproduz despacito"
- "pause": User wants to pause. Examples: "pausa", \
"pause", "para a música"
- "next": User wants next track. Examples: "próxima", \
"next", "skip", "pula"
- "previous": User wants previous track. Examples: \
"anterior", "previous", "volta", "back"
- "volume": User wants to change volume. Examples: \
"volume 50", "baixa o volume", "set volume to 80"
- "now_playing": User wants to know what's playing. \
Examples: "o que está tocando?", "what's playing?", \
"que musica é essa?"
- "add_to_playlist": User wants to add current track \
to playlist. Examples: "adiciona à minha playlist", \
"add to playlist", "save this song"
- "connect_spotify": User wants to connect their Spotify \
account. Examples: "conectar spotify", "connect spotify", \
"link spotify"
...
{{"action": "play_track", "query": "Bohemian Rhapsody", \
"language": "pt"}}
- "play Candy Shop" -> \
{{"action": "play_track", "query": "Candy Shop", \
"language": "en"}}
- "pausa" -> \
{{"action": "pause", "query": "", "language": "pt"}}
- "pause" -> \
{{"action": "pause", "query": "", "language": "en"}}
- "próxima" -> \
{{"action": "next", "query": "", "language": "pt"}}
- "volume 50" -> \
{{"action": "volume", "query": "50", "language": "en"}}
- "o que está tocando?" -> \
{{"action": "now_playing", "query": "", "language": "pt"}}
- "conectar spotify" -> \
{{"action": "connect_spotify", "query": "", \
"language": "pt"}}
- "who is Queen" -> \
...
class MusicAgent(BaseAgent):
    ...
    async def _handle_playback(
        ...
        """Handle playback actions requiring user OAuth."""
        ...
        player = SpotifyUserService(token)
        ...
        if action == "play_track":
            ...
            if spotify and spotify.is_initialized():
                ...
                if results:
                    track = results[0]
                    uri = track.get("uri", "")
                    success = await player.play(uri=uri)
                    if success:
                        return AgentResponse(
                            status=AgentStatus.SUCCESS,
                            response=t(
