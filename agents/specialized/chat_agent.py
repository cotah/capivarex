...
class ChatAgent(BaseAgent):
    ...
    async def execute(self, prompt: str, context: Dict[str, Any]) -> AgentResponse:
        ...
        # RAG: inject semantically relevant memories into system prompt
        if user_id:
            try:
                ...
                if rag_block:
                    ...
                    self.logger.info(
                        f"RAG: injected {len(memories)} memories for user={str(user_id)[:8]}"
                    )
            except Exception as e:
                self.logger.debug(f"RAG retrieval failed (non-fatal): {e}")
