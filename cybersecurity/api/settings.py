...
async def update_settings(body: SettingsUpdate):
    ...
    if body.fast_scan_interval is not None:
        if body.fast_scan_interval < 60:
            raise HTTPException(
                status_code=400, detail="fast_scan_interval must be >= 60s"
            )
    ...
    if body.medium_scan_interval is not None:
        if body.medium_scan_interval < 300:
            raise HTTPException(
                status_code=400, detail="medium_scan_interval must be >= 300s"
            )
    ...
    if body.slow_scan_interval is not None:
        if body.slow_scan_interval < 1800:
            raise HTTPException(
                status_code=400, detail="slow_scan_interval must be >= 1800s"
            )
    ...
    if body.alert_threshold is not None:
        if not 0 <= body.alert_threshold <= 1:
            raise HTTPException(
                status_code=400, detail="alert_threshold must be 0.0-1.0"
            )
    ...
    if body.autofix_suggest_threshold is not None:
        if not 0 <= body.autofix_suggest_threshold <= 1:
            raise HTTPException(
                status_code=400, detail="autofix_suggest_threshold must be 0.0-1.0"
            )
