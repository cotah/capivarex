...
async def detect_regressions() -> list[dict]:
    ...
    try:
        ...
        fixed_result = (
            ...
            .select("scanner, finding_type, file_path, title, resolved_at")
            .eq("status", "fixed")
        ...
        open_result = (
            client.table("cyber_findings")
            .select("id, scanner, finding_type, file_path, title, first_seen")
            .eq("status", "open")
            .execute()
        )
...
async def detect_escalating_threats(window_days: int = 7) -> list[dict]:
    ...
    try:
        ...
        current_result = (
            client.table("cyber_findings")
            .select("finding_type, first_seen")
            .gte("first_seen", current_start)
            .execute()
        )
        ...
        previous_result = (
            client.table("cyber_findings")
            .select("finding_type, first_seen")
            .gte("first_seen", previous_start)
            .lt("first_seen", current_start)
            .execute()
        )
        ...
        for ftype, current_count in current_counts.items():
            ...
            if current_count > 2 and (
                previous_count == 0 or current_count >= previous_count * 2
            ):
                escalating.append(
                    {
                        ...
                        "increase_factor": round(
                            current_count / max(previous_count, 1), 1
                        ),
                        "message": (
